...
abstract class HtmlInputElement extends HtmlFormControlElement
{
	...
	* @param boolean $disabled		(Optional) new values cannot be added to disabled controls
	* @param string $requestMethod	(Optional) GET or POST - if not set see HtmlSettings
	*/
	public function __construct($name, $type, $value = null, $disabled = false, $requestMethod = null)
	{
		parent::__construct($name, $disabled);
		$userValue = $this->GetUserValue($requestMethod);
		$this->Type = $type;
		$this->SetValue(($userValue === null) ? $value : $userValue);
	}
